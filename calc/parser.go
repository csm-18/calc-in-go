...
func add_subtract(tokens []Token) (error, []Token) {
	x := 0
	for x < len(tokens) {
		if len(tokens) == 1 && tokens[x].token_type == NUM {
			return nil, tokens
		} else if x != 0 && x+1 < len(tokens) && (tokens[x].token_type == PLUS || tokens[x].token_type == MINUS) {
			...
			result := 0.0
			if tokens[x].token_type == PLUS {
				result = left_operand + right_operand

			} else if tokens[x].token_type == MINUS {
				result = left_operand - right_operand
			}
